...
app.post('/save', async function (req, res) {
  ...
  const mode = req.body.mode;
	...
	const response = await accessSpreadsheet(duration, count, bank, dateEnd, init_bank, mode)
		.then(() => {
		...
		.catch(err => {
			...
			console.log(err);
			console.log('server: data NOT received at server');
		})
...
async function accessSpreadsheet(duration, count, bank, dateEnd, init_bank, mode) {
	//spreadsheet ID
